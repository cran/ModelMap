...
@
\end{center}
\caption{\label{fig:Ex2CompImpPA}Example 3 - Relative variable importances (permutation based) for predicting Presences verses predicting Absences for Pinyon ans Sage.}
\end{figure}
...
Make Out-Of-Bag model predictions on the training data and run the diagnostics on these predictions.  Save JPEG, PDF, and PS versions of the diagnostic plots. 

Out of Bag model predictions for a Random Forest model are not stochastic, so it is not necessary to set the seed.

Since this is a categorical response model model diagnostics include a CSV file the observed and predicted values, as well as a CSV file of the confusion matrix and its associated Kappa value. 
...
This table is designed to be easily interpreted in Excel, but is not very workable for carrying out analysis in R.  The \code{model.diagnostics()} function also outputs a CSV file of the observed and predicted values. 
...
To calculate the confusion matrix from this file we will use the observed and predicted columns. The \code{read.table()} function will convert columns containing character strings to factors. If the categories had been numerical, the \code{as.factor()} function can be used to convert the columns to factors. Because there may be catergories present in the observed data that are missing from the predictions (and vice versa), to get a symetric confusion matrix it is important to make sure all levels are present in both factors. 
...
To find Kappa for the confusion matrix, the \pkg{PresenceAbsence} package function \code{Kappa()} is used. Note that while most of the functions in the \pkg{PresenceAbsence} package are only applicable to binary confusion matrices, the \code{Kappa()} function will work on any size confusion matrix.
...
As in continuous and binary models, the \code{model.diagnostics()} function creats the variable importance graph \mbox{(Figure \ref{fig:Ex3VarImp}).} 
 
 \begin{figure}
\begin{center}
\includegraphics{VModelMapEx3_pred_importance}
\end{center}
\caption{\label{fig:Ex3VarImp} Example 3 - Variable importance graph for vegetation category.}
\end{figure}
...
Import the map output and transform the map output values to the row indices of the map codes. 

Note that here in example 3, where the categories are character strings, the integer codes from from column three are already the same as the row indices from column one, so this step is not strictly neccessary. However, if the categories in the training data had been unevenly spaced numeric codes, for example, \code{c(30,42,50,80)}, then \code{model.mapmake()} would keep the original values in the output. In this case, to produce a map in R with the \code{image()} function (as opposed to viewing the image in a GIS environment) it is neccessary to replace each category name with an evenly spaced index number in order to match each category to a color.

<<Ex3 Import data, results=hide>>=

mapgrid <- read.asciigrid(paste(MODELfn,"_map.txt",sep=""),as.image=TRUE)
mapgrid$z[]<-MAP.CODES$row[match(mapgrid$z[],MAP.CODES$integercode)]
@
...
image(mapgrid, col = MAP.CODES$colors,zlim=c(1,nrow(MAP.CODES)),asp=1,bty="n",xaxt="n",yaxt="n")
...
<<Ex4 Define numrows, results=hide>>=
numrows=500
@
